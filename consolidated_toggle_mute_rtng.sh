...
###############################################################################
# GET RADIOTRAY STATE (single query)
###############################################################################

RT_STATE=$(qdbus com.github.radiotray_ng /com/github/radiotray_ng \
com.github.radiotray_ng.get_player_state)

st=$(jq -r '.station' <<< "$RT_STATE")
muted=$(jq -r '.mute' <<< "$RT_STATE")
...
toggle_icon() {
    ...
    case "$1" in
        muted)
            [[ -f "$CONKY_DIR/xmuted.png" ]] && \
            mv "$CONKY_DIR/xmuted.png" "$CONKY_DIR/muted.png"
        ;;
        unmuted)
            [[ -f "$CONKY_DIR/muted.png" ]] && \
            mv "$CONKY_DIR/muted.png" "$CONKY_DIR/xmuted.png"
        ;;
    esac
}
...
mute_app() {
    ...
    stream=$(radiotray_stream)
    [[ -z "$stream" ]] && return

    wpctl set-mute "$stream" toggle
    toggle_icon muted
}
...
check_top_of_hour() {
    ...
    for s in "${stations[@]}"; do
        ...
        IFS=':' read -r file times length <<< "$s"

        if [[ -f "$file" && " $times " =~ " $minute " ]]; then
            top_of_hour_adbreak "$length"
        fi
    done
}
